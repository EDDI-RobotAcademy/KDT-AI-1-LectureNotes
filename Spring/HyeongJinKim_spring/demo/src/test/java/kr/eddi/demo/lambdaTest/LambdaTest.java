...
public class LambdaTest {
    ...
    왜(Why) ?
    왜에 대해 생각하기 전에 TDD는 Test를 먼저 작성하고
    이 'Test에 대응하는 코드를 만들어보자' 가 핵심이라고 보면 됩니다.

    이 내용을 기반으로 'Why' 에 대한 유추를 해보면
    - Test를 작성하면서 Domain을 파악할 수 있기 때문입니다.

    무엇이든 처음 접근하는 것은 매우 어렵습니다
    하지만 개발자는 처음 접근하는 것에 도전하며 깨부수는 직업입니다.
    ...
    오히려 위의 요구사항을 완벽하게 만들려다가 아무것도 못하고 나락으로 향하게 됩니다.
    이것의 대표적인 특징 중 하나가 주구장창 회의만 하는 회사나 그룹입니다.
    ...
    이렇게 하면 더 좋았을 것 같은데가 결국 Backlog 작성하는 관점에서 Review로 가는 것입니다.

    이 이야기는 TDD와 DDD에 연계된 철학속에 내포된 내용이고
    결론만 기억하고 싶다면 'Test가 Domain 찾기를 서포트한다' 라고 정의하면 됩니다.

    그리고 이것 때문에 순서가 바뀌는 것이 하나 있습니다.
    TDD를 시작하게 되면 일단 실패하는 테스틑 코드를 먼저 만들게 됩니다.
    그리고 그 실패하는 테스트 코드를 작성하기 위해
    필요한 로직들을 작성하여 최종적으로 Test가 구동되게 합니다.
    ...
    3. 테스트 코드가 동작하여 테스트를 완료되는지 검사합니다.
    4. 실패한다면 2~3번을 반복합니다.
