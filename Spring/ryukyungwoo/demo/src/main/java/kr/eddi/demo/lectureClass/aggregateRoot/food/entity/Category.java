...
@Table(name = "categoryType") // 이 엔티티와 데이터베이스 테이블을 매핑합니다. 여기서는 테이블 이름을 "categoryType"으로 지정합니다.
@NoArgsConstructor // Lombok을 사용하�� 기본 생성자 (인자가 없는 생성자)를 자동으로 생성합니다.
public class Category {
    ...
    private CategoryType categoryType; // CategoryType이라는 Enum 타입의 필드입니다.

    // CategoryType 인자를 받아 객체를 생성하는 생성자입니다.
    public Category(CategoryType categoryType) {
        this.categoryType = categoryType;
    }

    // CategoryType값을 반환하는 getter 메소드입니다.
