...
//        결론적으로 변수를 만들 때 가장 중요한 것은 변수 이름인데
//        명시성을 통해 함께 작업하는 팀원들에게
...
//        문자열 + 숫자의 경우엔 앞의 문자열이 나왔기 때문에
//        자동으로 숫자를 문자열 처리하여 화면에 출력한다.
        System.out.println("totalCount = " + totalCount);

//        final을 사용하는 이유 : 우선 상수로 고정시킬 수 있다는 이점이 있다. ★(유지보수)
//                상수로 고정시킬 수 있다는 것의 이점이란 무엇인가?
//        1. 아래 있는 [ 비교 대상 ] 을 보면 3.3f 를 직접 기입하고 있으므로
//           향후 프로그램이 커지면 직접 3.3f를 작성한 부분을 모두 찾아서
//           TAX 변경에 따라 모든 코드를 수정해야 하는 불편함이 발생한다.
//           반면 TAX 상수에 숫자를 기입하고 이 상수를 사용한다면
//           변동 상황이 발생할 때 해당하는 TAX 수치 값만
//           한 번 변경하면 모든 작업이 일괄처리 되므로 편리하다.
...
//      신입이 조회를 만들고 있는 상황이다.
//      만들던 중 코드에 계속, 1과 2가 날아오니까 신입은 그냥 들어온 숫자를 3으로 대입하고 문제를 해결했다.
//      이 상황에선 예금을 했더니 조회가 되고, 출금을 했더니 조회가 되고, 조회를 했더니 조회가 되는 끔찍한 상황 발생 !
//      이것을 원천 차단하는 방법으로서 final을 사용한다.
