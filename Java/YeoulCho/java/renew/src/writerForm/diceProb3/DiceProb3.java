...
public class DiceProb3 {
    ...
    // 음 우선 게임 객체생성자에서 뭐 필요한가? 모르겠네
    // 2번의 게임플레이()를 만들어보자!
    // 일단 플레이어가 주사위를 굴려야겠네?
    // 이상태에서 진행하려고 했더니 플레이어와 주사위가 없네
    // [1]초기에 셋팅할까? <- 생성자인 1번에 필요한것들을 배치
    // [2]그냥 여기서 다 때려박을까? <- 2번흐름에서 필요한 정보를 배치
    ...
    // 이때 가서 내가 조건에서 뭔가 빠뜨린게 있나? 를 생각해봅니다.

    // 어? 보니까 이런 조건이 있었네
    // [첫 번째 주사윅 짝수면 나머지 2개를 굴릴 수 있음]
    ...
    // 오 좋은데? 근데 뭔가 이상한데?
    // 조건을 다시 보니까 특수 기능이 동작하지 않은 것을 확인했습니다.
    // 아래와 같은 특수기능들이 존재하고 있었습니다.

    // 숫자가 1이면 친구에게 3점 훔치기
    // 숫자가 3이면 버프2점
    // 숫자가 4라면 무조건 패배

    // 일단 세번 째 주사위의 숫자를 확인합니다.
    // System.out.println()에 찍어서 잘나오나 확인해 봅시다.
    // 잘 되는 것 같았는데 근데 갑자기 에러가 튀어나오네요?
    // 아마도 에러 메시지를 읽어보면 NullPointerException 혹은
    // index outOfBound 같은것에 해달할 것입니다.

    // 어? 왜안나오는거야?
    // 살펴보니까 아 두번째와 세번째 주사위가 안나올 수가 있었네?
    // 이부분을 고려하지 않았구나!
    // 예외처리 추가해야겠네!
    ...
    // 우선 숫자 4번부터 보기로 했으니까 사망판정부터 때립니다.

    // 주사위 숫자를 더하다보면 음수가 나올 수 없으니까 그냥 음수값 큰거 주자!
    // 숫자가 4가 나오면 사용자 점수에 대략 -777점 같은 형태로 점수를 줍니다.
    // 여기까지 처리하고 실제 숫자4가 나올때 패배 처리가 되는지 확인합니다.

    // [숫자가 3이면 자가 버프 2점]
    // 이제 다음으로 만만한 자가 버프를 해봅시다.
    // 세 번째 주사위 숫자 3을 확인하면 자신의 점수에 2점을 추가해줍니다.
    // 이후 다시 돌려서 3이 나왔을 개 2점이 추가되는지 확인합니다.
    // 그 와 동시에 숫자4에 대한 패배처리도 확인합니다.

    // [숫자가 1이면 친구에게 3점 훔치기]
    // 이제 마지막으로 세번째 주사위 숫자가 1인 경우를 체크합니다.
    // 여기서는 상대편의 값을 훔쳐오기 때문에
    // 상대편을 파악할 수 있어야합니다.
    // 현재 내 상태를 기반으로 상대를 파악할 수 잇는 코드를 하나 작성합니다.
    // 그리고 잘 가져왓는지 System.out.println()으로 출력해서 확인해봅니다.

    // 이제 상대편을 파악했으니까 상대편 점수에서 3점 가져와서 상대편의 점수를 갱신합니다.
    // 그리고 상대편에서 뺏어서 자신에게 가져오는 것이므로
    // 3점을 자기 자신에게 더해서 갱신합니다.
    // 최종적으로 숫자 1,4,3에 대한 모든 처리가 잘 동작하는지 확인합니다.
