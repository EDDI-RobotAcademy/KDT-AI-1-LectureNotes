...
// Led 클래스는 불이 켜졌다 혹은 꺼졌다만 관리하면됩니다.
//클래스의 형태는 형재 아래와 같습니다.
/*
    -----  Led 객체 -----
    |      isTurnOn     |
    ---------------------
...
class Led{
    ...
    /*
    생성자는 class의 이름과 같습니다. 그리고 아래와 같이 리턴 타입이 없습니다
    만드는 규칙이라면 아래와 같습니다
    1. public을 적고 클래스 이름을 적은 이후 소괄호()를 작성후 중괄호{}를 작성합니다.
    2. 만약에 외부에서 값을 입력 받을 것이라면 소괄호에 입력받은 형태를 작성합니다
    3. 실제 클래스가 new를 통해 객체화 될 때 구동시키고 싶은 작업을 중괄호 내부에 배치합니다
    ...
    매서드를 작성하는 방법
    1. public을 작성하고 소괄호()를 작성하고 중괄호{}를 작성합니다
    ...
    4. 중괄호 내에서는 실제 매서드 이름에 해당하는 작업을 진행하면 됩니다
       ...
       3 -> [   ] -> 9 (리턴 타입 int)
       버튼 누름 -> [   ] -> ture (리턴 타입 boolean)
       1 ->[    ] -> "예금" (리턴 타입 String)
       회원정보 ->[    ] -> x (리턴 타입 void) == 정확히는 리턴하지 않음을 의미 / 생성자는 그냥 리턴 타입이 없음 둘 차이점 알기
            [   ] -> 20 (리턴타입 int)
    ...
    /*
    사실은 자기 자신을 켤 수 없기 떄문에 다른 객체의 도움을 받아야함
    수십년간의 SW 전문가들이 이러한 개념들을 정리하였고 그 개념의 일환으로 탄생하게 된 개념이 Domain Service 개념입니다
    비지니스 관점을 좀 더 명확하게 만들어준다는 이점이 있습니다
    실제로 전구를 키고 끄는 작업을 가지고 비지니스를 할만한 것들이 많지는 않지만 그래도 Domain Service를 나눠 본다면
    아래와 같은 것들이 존재할 것입니다
    켜기, 끄기, 깜빡이기
    
    조금 쉽게 접근해 보자면 Domain Service는 객체 스스로가 직접 하기에는 표현이 애매해지는 작업들을 모두 Domain Service로 
    재배치하게 됩니다. 이를 통해 가독성과 유지보수성의 향상을 가져올 수 있습니다
...
public class LectureClassTest {
    public static void main(String[] args) {
        ...
        하지만 모든 정보를 하나의 클래스에서 관리하게 되는 경우 객체가 비대해지면서 해당 객체가
        어떤 목적을 가지고 있었는지 목적성을 잃게 됩니다
        ...
        내가 직접 커스텀 할 수 있는 데이터 타입이 클래스라 보면 됩니다
         */
