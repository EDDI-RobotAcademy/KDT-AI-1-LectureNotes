...
public class RefactorManager2 {
    ...
    List<RefactorProduct2> productList;
    ...
    public void addRandomProduct() {
        ...
        for (int i = 0; i < productNameArray.length; i++) {
            ...
            productList.add(new RefactorProduct2(
                    productNameArray[i], vendor, price));
        }
    }

    public void printProductList () {
        for (RefactorProduct2 product: productList) {
    ...
    public void buyRandomProduct() {
        ...
        for (int i = 0; i < productLength; i++) {
            RefactorProduct2 product = productList.get(
    ...
    public void fineProductMember(){
        for(int i = 0; i < productList.size(); i++){
            System.out.println(productList.get(i).getProductName() + " 구매자 목록: ");

            for(int j = 0; j < productList.get(i).getMemberIdList().size(); j++){
                memberList.get(productList.get(i).getMemberIdList().get(j));

                if(memberList.get(productList.get(i).getMemberIdList().get(j)) == null){
                    continue;
                }
                System.out.println(memberList.get(productList.get(i).getMemberIdList().get(j)));
            }
        }
    }
}