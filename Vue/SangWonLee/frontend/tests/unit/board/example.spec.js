...
describe("requestCreateBoardToSpring", () => {
  test("Spring에 게시물 작성 요청 검증", async () => {
    ...
    await expect(
      storeMock.actions.requestCreateBoardToSpring({ title, writer, content })
    ).toStrictEqual(board);
  });
});
...
describe("requestBoardListToSpring", () => {
  test("Spring에 게시물 리스트 요청 검증", async () => {
    ...
    await expect(
      storeMock.actions.requestBoardListToSpring().length
    ).toStrictEqual(3);
  });
});
...
describe("requestBoardToSpring", () => {
  test("Spring에 게시물 읽기 요청 검증", async () => {
    ...
    await expect(
      storeMock.actions.requestBoardToSpring({ boardId })
    ).toStrictEqual([board]);
  });
});
...
describe("requestDeleteBoardToSpring", () => {
  test("Spring에 게시물 삭제 요청 검증", async () => {
    ...
    await expect(
      storeMock.actions.requestDeleteBoardToSpring({ boardId })
    ).toStrictEqual(true);
  });
});
...
describe("requestBoardModifyToSpring", () => {
  test("Spring에 게시물 수정 요청 검증", async () => {
    ...
    await expect(
      storeMock.actions.requestBoardModifyToSpring({ title, content, boardId })
    ).toStrictEqual([board]);
  });
});
