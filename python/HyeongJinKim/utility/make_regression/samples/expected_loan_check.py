...
# 수식을 세우기 위해 위쪽에서 계산한 값을 다시 정리해봅니다.
# 10^8 * (소득[비율] - 1.932 - 3)
# 소득[비율] = 실제 소득 / 10^8
...
# 그리고 현재 시스템이서 최소값은 800만원으로 지정하였고
...
# 0.75
# 7000 - 1932 = 5000
# 실제 최대치인 4000만원을 한계로 지정한다면 0.70정도의 수치를 가짐
total_score = calculate_score(100000000, 0, 10000000)
print("정상 루틴 점수: {0}".format(total_score))

# np.arange 특성으로 float 처리가 안되므로
# 각 숫자에 1000만을 곱하고 해당 값에 대해 다시 1000을 나눠서 값을 산출
score_list = np.linspace(4142, 7046, 32)
print(score_list)

# res = 800
# for i in range(len(score_list) - 1):
#     start = (int)(score_list[i])
#     end = (int)(score_list[i + 1])
#     res += 100
#     # print("0.{0} ~ 0.{1} -> {2}".format(start, end, res))
#     print("0.{0}: {1}0000,".format(start, res))
