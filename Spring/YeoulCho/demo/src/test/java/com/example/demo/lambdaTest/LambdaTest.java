...
public class LambdaTest {
    ...
    그리고 이 천상계로 가기 위해 반드시 필요한 작업들이 TDD,DDD입니다.

    실제로 Java 코드를 작성 할 때도 문제 전체를 한번에 만들지 않고
    여기서 문제 자체가 요구하는 사항들을 정리하였습니다.
    -여기서 또 팁이라면 요구 사항을 정리할 때 완벽할 필요가 없습니다.

    오히려 위의 요구 사항을 완벽하게 만드려다가 아무것도 못하고 나락으로 향하게 됩니다.
    이건의 대표적인 특징 중 하나가 회의만 하는 회사나 그룹입니다.
    회의 백날 해봐야 결론이 안나오고 여기에 쓸모없이 소요해야하는 시간과 비용이 매우 큽니다.
    실제로 이렇게 회의만 수 개월에서 수년을 하다가 아무것도 못하고 사라지는 경우가 많습니다.

    그래서 Facebook에서도 이런 이야기가 있습니다.
    "Done is better than perfect"
    일단 만들라는 것이죠.

    만들어 보면 스스로도 만든것에서 무엇이 노답인지가 보이기 때문입니다.
    이렇게 하면 더 좋았을 것 같은데?가 결국 Backlog작성하는 관점에서 Review로 가는 것입니다.
    ...
    TDD를 시작하면 되면 일단 실패하는 테스트코드를 먼저 만들게 됩니다.
    그리고 그 실패하는 테스트 코드를 작성하기 위해
    필요한 로직들을 작성하여 최종적으로 Test가 구동되도록 만듭니다.
    그래서 결론적으로 Test는 Backlog 그자제가 됩니다.
