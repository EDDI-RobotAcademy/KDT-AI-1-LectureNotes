...
/*
   ...
   /   get TurnOn  /      new를 생성하면 위의 is TurnOn이 false로 시작을 한다.
   /   Set TurnOn   /
   -----------------
...
class Led {  //생성자는 그냥 리턴타입이 없다. (아래의 설명 void랑 다르다)
    ...
    // 1)       3 -> [  ] -> 9 (리턴 타입 int)
    // 2)  버튼누름 -> [  ] -> true (리턴 타입 boolean)true가 튀어나왔으니까 리턴타입이 boolean일 것이다.
    // 3)       1 -> [  ] -> "예금" (리턴 타입 String)
    // 4)  회원정보 -> [  ]   (리턴 타입 void)- 리턴타입이 없는게 아니고 하지 않는것이다. 바깥으로 튀어나온게 없음. 회원정보만 받은 경우.
    // 5)             [  ]-> 20 (리턴 타입 int) - 입력을 하지 않은 것이다. 없는게 아니고

    public Boolean getTurnOn () {
        return isTurnOn;
    }

    // 자기 스스로 킬수 없다. 그래서 다른 객체의 도움을 받아야 함
    // 사실 수십년간 SW 전문가들이 이러한 개념들을 정리하였고
    ...
    // 자기가 직접 처리하기 애매한 작업들을 도메인서비스가 한다.
    ...
    // 즉 필요하면 Fianl 객체에 final 변수들을 설정해서 전달한다는 의미입니다.



    // * 입력 관점에서 살펴봅시다!
    // 1)       3 -> [  ] -> 9 (입력 타입 int)
    // 2)  버튼누름 -> [  ] -> true (입력 타입 Butteon class)
    // 3)       1 -> [  ] -> "예금" (입력 타입 int)
    // 4)            [  ] -> 20  (입력 타입 void)
    // 5)  참/거짓 -> [  ]         (입력 타입 boolean)

    public void setTurnOn (Boolean TurnOn) { //리턴이 없어서 void 들어옴. 쿼터타입을 잡고 아래에서 뭘 할지를 작성한다.
