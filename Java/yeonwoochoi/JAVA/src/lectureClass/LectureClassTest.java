...
class  Led {
    ...
    // 2. 마약에 외부에서 값을 입력 받을 것이라면 소괄호에 입력받을 형태를 작성합니다.
    ...
    // 사실 수십년간의 SW 전문가들이 이런한 개념들을 정리하였고
    // 그 개념의 일환으로 탄생하게 된 개념이 Domain Service 개념입니다.
    // Domain Service 를 만들어서 얻는 이점은
    // 비즈니스 관점을 좀 더 명확하게 만들어준다는 이점이 있습니다.
    // 실제로 전구를 키고 끄는 작업을 가지고 비즈니스를 할만한 것들이 많지만 않지만
    // 그래도 Domain Service 를 나눠 본다면 아래와 같은 것들이 존재할 것입니다.
    // 키기, 끄기, 깜빡이기
