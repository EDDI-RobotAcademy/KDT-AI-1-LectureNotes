<template>
    ...
    <div>
        ...
        <!-- span은 줄바꿈이 없음. 특정 스타일을 적용할 때 많이 사용하는 편-->
        ...
        <!-- 
            ...
            tr은 table-row에 해당하면 td는 table-data로 call 혹은 column이라 봐도 무방
        ...
        <table style="width: 600px">
            ...
            <caption>
                ...
                <p>Modern Development Method in Kotlin</p>
            </caption>
            <tr>
                <th>Name</th>
                <td>이름</td>
        ...
        <!-- 
            Image를 추가하고 싶을 때는 아래와 같이 이미지가 위치하는 경로를 지정해주면 됨
            부가적으로 download로 확보 했는지 아니면
            실질적으로 서비스 차원에서 사용자가 등록한 이미지인지 같은 것을 디렉토리 이름으로 구별하면 좋음
        -->
        <h2>Image</h2>
        <img src="@/assets/downloadImage/Pororo.jpg">
        <!-- figure로 image를 감싸고 figcaption을 달면 image에 대한 설명을 붙일 수 있음 -->
        <figure>
            <img src="@/assets/downloadImage/Pororo.jpg">
            <figcaption>와! 뽀로로다!</figcaption>
        </figure>
        <h1>Text Link 생성</h1>
        <a href="https://golang.org">Golang Homepage</a>
        <h1>이미지 링크 생성</h1>
        <a href="http://www.pororopark.com">
            <img src="@/assets/downloadImage/Pororo.jpg">
        </a>
        <!--
            form 태그는 말 그대로 format(양식)을 지정해서 데이터를 전송
            method에 post를 지정할 수 있고 default 값은 get
            text는 말 그대로 문자열, number는 숫자, submit은 등록 개념
            속성값 value를 통해 표기되는 형태를 지정할 수 있음
            추후 이 부분의 submit을 prevent하여 vue가 axios하도록 구성하게 됨
        ...
        <!--
            다중 선택(checkbox)
            input type으로 checkbox를 사용하여 value 값에 선택한 값을 볼 수 잇음
            javascript에서 어떤 값을 선택했는지 처리할 수 있고 
            이를 토대로 vue쪽에 axios하는 데이터를 처리할 수 있음
        -->
        <h3 style="text-align: left;">다중 선택</h3>
        ...
        <!--
            단일 선택(radio)
            input type으로 radio를 사용하여 value 값에 선택한 값을 볼 수 잇음
            단일 선택이 되도록 명확하게 지정하려면 name값을 모두 같은 것으로
            지정하여 중복 선택이 불가하게 만듦
            마찬가지로 javascript에서 어떤 값을 선택했는지 처리할 수 있고 
            이를 토대로 vue쪽에 axios하는 데이터를 처리할 수 있음
        -->
        <h4 style="text-align: left;">단일 선택</h4>
        <form action="/" style="text-align: left;">
            <li>
                ...
                <ul>
                    ...
                    <li>
                        ...
                        <!-- 
                            ...
                            최솟값, 최댓값, 초기값, step 설정은 위의 number와 동일
                        -->
            ...
            <fieldset>
                ...
                <ul>
                    <li>
                        ...
                        <!-- select의 경우엔 드롭 다운 형태의 목록을 만들 수 있음
                        multiple을 선택하여 여러 옵션들을 복수 선택 할 수 있음
                        size는 눈에 보이는 리스트의 개수-->
                        <select size="5" id="subject">
                            <!-- option에는 실제 배치하고 싶은 옵션 리스트를 작성 -->
                            <option value="archi">소프트웨어 아키텍처</option>
                            <option value="ddd">DDD 설계</option>
                            <option value="clan">Clean 아키텍처</option>
