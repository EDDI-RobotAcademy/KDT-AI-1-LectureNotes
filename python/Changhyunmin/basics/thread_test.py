...
def thread_test_function(s, pool):
    logging.debug('풀에 조인하기 까지 대기중')
...
# cpu 의 물질적 갯수는 정해져 있는데 그것을 사용하고 싶은 proses 들은 너무 많기에
# 1번 proses가 동작중에 2번, 3번 proses가 동작하려고 끼어들어서 발생한 문제이다.
# 여기서 문제는 정보가 공유되서 문제가 되는거다.
