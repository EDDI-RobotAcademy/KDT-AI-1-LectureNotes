...
public class VariableTest {
    public static void main(String[] args) {
        ...
        // 3. 필요하다면 초기화를 진행한다.
        // 변수를 만들 때 가장 중요한 것은 변수 이름인데 명시성을 통해 함께 작업하는
        // 팀원들에게 이것이 무엇을 의미하는 것인지 명확하게 전달하기 위한 목적이 가장 중요합니다.
        int appleCount = 3;
        int grapeCount = 5;
        int totalCount = appleCount + grapeCount;
        // 문자열 + 숫자의 경우엔 앞에 문자열이 나왔기 때문에 자동적으로 숫자를 문자열 처리하여 화면에 출력합니다
        System.out.println("totalCount = " + totalCount);

        // 상수 final 이점 1
        // 상수로 고정시킬 수 있다는 것의 이점이란 무엇인가?
        // 1. 아래 있는 [비교대상] 을 보면 3.3f를 직접 기입하고 있으므로
        // TAX 변경에 따라 모든 코드를 수정해야 하는 불편함이 발생합니다.
        // 반면 TAX 상수에 숫자를 기입하고 이 상수를 사용한다면
        // 변동 상황이 발생할 때 해당하는 TAX 수치값만
        // 한번 변경하면 모든 작업이 일관처리 되므로 편합니다.
        ...
        // TAX = TAX  + 4;
        // 위 코드는 TAX final 이기 때문에 새로운 값을 대입하거나 덧셈, 뺄셈 등등이 불가능합니다.
        // 결론적으로 입력되는 값을 변경하지 못하게 막음으로서 원래 동작해야 하는 동작의 무결성을 보장하게 됩니다.

        // 예) 1 - 예금, 2 - 출금, 3 - 조회
        // 회사 프로그램이 만들어져 있는 상태(예금 , 출금)
        // 신입이 조회를 만들고 있는 상황입니다.
        // 만들던 중 코드에 계속, 1과 2가 날아오니까 신입은 그냥 들어온 숫자를 3으로 대입하고 문제를 해결했습니다.
        // 이 상황에선 예금을 했더니 조회가 되고 출금을 했더니 조회가 되고 조회를 했더니 조회가 되는 끔찍한 상황이 연출됩니다.
