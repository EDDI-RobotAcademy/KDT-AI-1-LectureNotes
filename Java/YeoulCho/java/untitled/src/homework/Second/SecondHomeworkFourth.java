...
public class SecondHomeworkFourth {
    public static void main(String[] args) {
        int diceNumber1 = (int)(Math.random() * 6 + 1) +1;
        int diceNumber2 = (int)(Math.random() * 6 + 1) +1;
        int diceNumber3 = (int)(Math.random() * 6 + 1) +1;
        int diceNumber4 = (int)(Math.random() * 6 + 1) +1;
        ...
        for ( int i =0; i<diceNumberArray.length; i++){
            diceNumberArray[i] = (int)(Math.random() * 6 + 1) +1;
        }

        //어쨋든 배열을 만들었는데 숫자가 적혀 있어서 싫음
        int MAX_DICE_NUMBER1 = 12;

        int[] diceNumberArray2 = new int[MAX_DICE_NUMBER1];
        for (int i =0 ; i<MAX_DICE_NUMBER1; i++){
            diceNumberArray2[i] = (int)(Math.random() * 6 + 1) +1;
        }
        ...
        for (int i =0 ; i<MAX_DICE_NUMBER; i++){
            ...
            //그리고 MIN=3, MAX=6이면
            //현재 코드에서 3~8까지 표현되겠네?
            //diceNumberArray3[i] = (int)(Math.random() * 6 + 1) +1;
