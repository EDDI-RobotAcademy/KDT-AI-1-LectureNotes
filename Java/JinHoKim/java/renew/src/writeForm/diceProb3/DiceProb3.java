...
public class DiceProb3 {
    public static void main(String[] args) {
        ...
        테스트를 만들 때도 이런 개념을 만들면서 점진적 확장을 시작하면 됩니다.
        1. 게임객체() 생성
        2. 게임객체. 게임플레이()
        3. 게임객체. 승자판정()

        우선 게임 객체 생성자에서 뭐 필요한가? -> 모름
        2번의 게임플레이()를 만들어보자
        일단 플레이어가 주사위를 굴려야겠네?
        이 상태에서 진행하려고 했더니 플레이어와 주사위가 없네?
        [1] 초기에 셋팅할까?  ==> 생성자인 1번에 필요한 것들을 배치
        [2] 그냥 여기에 전부 넣을까? ==> 2번 흐름에서 필요한 정보를 배치
        ...
        2번에서 주요한 역할인 주사위 굴리기가 끝났으니까 3번에 해당하는 승자 판정을 시작해보려고 합니다
        ...
        어? 보니까 이런 조건이 있었네?
        [첫 번째 주사위가 짝수면 나머지 2개를 굴릴 수 있음]
        주사위 굴리는 부분에 첫 번째 주사위가 짝수로 판정되는 경우에만 나머지 2개를 돌리도록 코드를 변경하자
        ...
        세 번째 주사위가 있는지 없는지 검사합니다
        일단 세 번째 주사위의 숫자를 확인합니다
        System.out.println()에 찍어서 잘 나오나 확인해 봅시다
        잘 되는 것 같았는데 갑자기 에러가 튀어나오네요?
        아마도 에러 메시지를 읽어보면 NullPointerException 혹은 index OutOfBound 같은 것에 해당할 것입니다

        어? 왜 안 나오는 거야?
        살펴보니까 두 번째와 세 번째 주사위가 안나올 수 있었네?
        이 부분을 고려하지 않았구나
        예외처리 추가해야겠네
        세 번째 주사위 숫자를 확인하기 전에 해당 주사위가 존재하는지 먼저 확인해야겠구나

        실제 읽은 숫자가 어떤 조건에 대응하는지 살펴봐야합니다
        우선 숫자 4번부터 보기로 했으니까 사망 판정부터 때립시다

        주사위 숫자를 더하다보면 음수가 나올 수 없으니까 그냥 음수값 큰거 주자
        숫자가 4가 나오면 사용자 점수에 대략 -444 점 같은 형태로 점수를 줍니다.
        ...
        여기서는 상대편의 값을 훔쳐오기 때문에 상대편을 파악할 수 있어야 합니다
        ...
        그리고 상대편에서 뺏어서 자신에게 가져오는 것이므로 3점을 자기 자신에게 더해서 갱신합니다
        최종적으로 숫자 1, 3, 4에 대한 모든 처리가 잘 동작하는지 확인합니다

        만약 과정 중간에 문제가 발생한다면 문제의 원인을 파악해서 점진적으로 개선합니다
         */
    }
}
