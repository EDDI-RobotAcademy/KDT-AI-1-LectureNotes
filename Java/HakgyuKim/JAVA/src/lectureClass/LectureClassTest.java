...
// Led 클래스는 불이 켜졌다 혹은 꺼져다만 관리하면 됩니다.
...
class Led {
    ...
    // 2. 만약에 외부에서 값을 입력 받을 것이라면 소괄호에 입력받을 형태로 작성합니다.
    // 3. 실제 클래스가 new를 통해 객체화 될 때 구동이키고 싶은 작업을
    //    중괄호 내부에 배치합니다.
    ...
    // 3. 매서드의 이름을 그 옆에 작성해주니다.
    //    이름은 역시나 이 작업에 무엇을 하는지 명시적으로 알려줄 수 있는 형태가 좋습니다.
    ...
    //       1 -> [    ] -> "예금" (리턴 타입 String)
    // 회원 정보 -> [    ]          (리턴 타입 void) == 정확히는 리턴하지 "않음"을 의미함
    //            [    ] -> 20     (리턴 타입 int)
