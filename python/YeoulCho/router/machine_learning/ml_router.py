...
async def create_virtual_credit():
    ...
    # 가우시안 분포(정규분포)를 따르는 확률 밀도 함수를 생성함 -> np.random.normal()
    # 확률의 종류가 여러가지 있습니다.
    ...
    # np.random.normal()에 평균과 표준편차를 입력해서 해당 함수를 따르는 값들을 만들어 달라 요청하는 함수 입니다.
    ...
    virtual_credit_date = pd.DataFrame({
        '나이': age,
        '소득': income,
        '연체율': arrears_rate
    })
