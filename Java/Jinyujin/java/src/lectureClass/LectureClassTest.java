...
/*
        ...
        -----------------------
 */
//... new를 하면 생성자 만들어지고 false로 만들어 시작할 때 꺼져있게 됨
class Led {
    private Boolean isTurnOn;

    //... alt + insert 누르면 생성자 나온다/ 직접 쳐도 되긴 함
    ...
    // 3. 실제 클래스가 new를 통해 객체화 될 때 구동시키고 싶은 작업은
    //    중괄호 내부에 배치합니다.
    ...
    // 클래스 내부에 뭔가 기능을 수행하는 집합들을 매서드라고 부릅니다.
    // 매서드를 작성하는 방법
    ...
    //... 튀어나오는 유형에 따라서 리턴 타입이 정해진다

    // 3 -> [   ] -> 9 여기서 []안에 무엇을 들어갔을 때 바깥으로 뭔가 튀어나오는 것 (리턴 타입 int)
    //... 숫자가 튀어나왔으니 int가 되면 되는 것
    // 버튼 누름 -> [   ] - > true (리턴 타입 boolean)
    //... 리턴  타입이 boolean 이라는 뜻
    // 1 -> [   ] -> "예금" (리턴 타입 string)
    //... 숫자를 보고 예금이라는 문자가 튀어나옴
    // 회원 정보 -> [   ] (리턴 타입 void) == 정확히는 리턴하지 않음을 의미함 (리턴이 없는게 아니라 하지 앖은 것)
    //... 회원 정보를 들어가는데 반환된게 없는 것
    //... 생성자는 리턴 하지 않는게 아니라 리턴 타입이 없음!!! 주의 void와 다르다는 것
    //   [   ]  -> 20 (리턴 타입 int)
    //... 입력을 하지 않은 케이스
    ...
    // 실제로 전구를 키고 끄는 작업을 가지고 비즤스를 할만한 것들이 많지는 않지만
    ...
    // 이런 관점으로 접근하면 settter를 완벽하게 제거할 수 있습니다.
...
public class LectureClassTest {
    public static void main(String[] args) {
        ...
        // Domain이라는 관점은 렇게 클래스가 어떤 주제에 집중을 하고 있는지를 본다고 생각하면 됩니다.
        // 즉 내가 집중하는 주제가 무엇인가를 알 수 있도록 예쁘게 잘 표현해주는 것을 OOP라 봐도 무방합니다.

        //... 레고 - 이거 붙인 다음 뭐 붙일까 생각
        // 이런게 분별이 안되면 조각낸 의미가 없어짐
