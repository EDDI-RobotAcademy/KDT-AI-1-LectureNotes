...
min_load_amount = 30000000
max_load_amount = 45000000
step_size = 1000000

num_samples = 1000
# 미상환 금액
random_outstanding_amounts = np.random.randint(min_load_amount, max_load_amount + 1, size=num_samples) * 10000
# 미상환 건수
random_outstanding_counts = np.random.randint(1, 10, size=num_samples)
# 연체율
random_delinquency_rates = np.random.uniform(0.0, 0.3, size=num_samples)
...
for i in range(num_samples):
    # outstanding_amounts는 OK, 나머지는 amount를 계산할 때 직접 계수값을 곱해서 조정하도록 해줘야 함
