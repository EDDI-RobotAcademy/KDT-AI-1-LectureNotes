...
async def create_virtual_credit():
    ...
    std_dev_arrears_percent = 0.025

    data_size = 1000

    #가우시안 분포(정규 분포)를 따르는 확률 밀도 함수를 생성함 -> np.random.normal()
    #확률의 종류가 여러가지 있습니다.
    #푸아송 분포, 이항 분포, 기하 분포, 정규 분포, 등등이 있다.
    #그 중 자연을 번영하는 확률 분포가 가우시안 분포에 해당한다.
    #exp^-x^2형태의 함수를 가지고 있고 x^2측에 평균과 표준편차로 그래프 모양이 결정됩니다.
    #np.random.normal()에 평균과 표준편차를 입력해서 해당 함수에 따르는 값들을 만들어달라 요청하는 함수입니다.

    #그래프 추이를 보고 싶다면
    #wokfram alpha로 이동합니다.
    #수식 편집기에 다음 내용을 입력하세요
    # y= exp(-((x -70) / (sqrt(2)*100.5))^2)
    #여기서 70은 편균[mean] 값에 해당합니다.
    #그리고 100.5는 표준편차[standard deviton]에 해당합니다.
