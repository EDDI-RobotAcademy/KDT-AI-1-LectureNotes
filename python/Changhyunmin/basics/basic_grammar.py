def python_basics():
    ...
    print("[0:1]:", test_str[0:4])
    print("[:2]:", test_str[:2])
    print("[-2:]:", test_str[-2:])
    print("[:]:", test_str[:])
    print("[::2]:", test_str[::2])
    print("[1::9]:", test_str[1::9])
    ...
    # - 불변 특성을 가지고 있는 리스트
    # 그럼 이 녀석은 보토 어디서 마주하게 될까 ?
    # - python에서 리턴시 무조건 tuple이 떨어지게 됩니다.

    (a, b) = (3, 7)
    print(a, b)
    # java에선 템플을 해서 바꿨어야 하지만 python은 바로 바뀐다.
    a, b = b, a
    print(a, b)
    # Spring의 JSON, java의 hash 와 같은 느낌이다
    d = dict(a=1, b=3, c=5)
    print(d)
    print(type(d))
    ...
    for fruits_color in fruits.values():
        print(fruits_color)
    ...
    # java: List<Integer> numList = new ArrayList<>();
    # for (int i = 1; i < 6; i++) {
    #   numList.add(i * i);
