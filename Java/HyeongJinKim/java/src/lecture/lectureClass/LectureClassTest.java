...
class Led {
    ...
    // 만드는 규칙이라면 아래와 같습니다.
    // 1. public을 적고 클래스 이름을 적은 수 소괄호()를 작성 후 중괄호{}를 작성합니다.
    ...
    // 사실은 전구 스스로 켜질 수 없기 때문에 다른 객체의 도움을 받아야 한다.
    // 수십년간의 SW 전문가들이 이러한 개념들을 정리하였고
    // 그 개념의 일환으로 탄생하게 된 개념이 Domain Service 개념입니다.
    // Domain Service를 만들어서 얻는 이점은
    // 비즈니스 관점을 좀 더 명확하게 만들어준다는 이점이 있습니다.
    // 실제로 전구를 켜고 끄는 작업을 가지고 비즈니스를 할만한 것들이 많지는 않지만
