...
public class DiceProb3 {
    public static void main(String[] args) {
        ...
        지금은 관심사를 세부사항이 아닌 전체흐름을 잡는 것에 집중하도록 합시다
        ...
        [2]그냥 여기서 다 때려박을까? <- 2번 흐름에 필요한 정보를 배치
        ...
        * 주사위를 굴립니다.
        ...
        이 때 가서 내가 조건에서 뭔가 빠뜨린게 있나? 를 생각해봅시다
        ...
        오 좋은데? 근데 뭔가 이상한데?
        조건을 다시 보니까 특수 기능이 동작하지 않는 것을 확인하였습니다
        아래와 같은 특수 기능들이 존재하고 있습니다.
        ...
        sout에 찍어서 잘 나오는지 확인합니다
        잘 되는 것 같았는데 근데 갑자기 에러가 튀어나오네요?
        아마도 에러 메세지를 읽어보면 NullPointException 혹은
        index OutOfBound 같은 것에 해당할 것입니다.
        ...
        해당 주사위가 존재하는지 먼저 확인해야 겠구나

        실제 읽은 숫자가 어떤 조건에 대응하는지 살펴봐야합니다.
        우선 숫자 4번부터 보기로 했으니까 사망판정을 때립시다.

        주사위 숫자를 더하다보면 음수가 나올 수 없으니까 그냥 음수값 큰거 주자!
        숫자 4가 나오면 사용자 점수에 대략 -777 점 같은 형태로 점수를 줍시다.
        여기까지 처리하고 실제 숫자 4가 나올때 패배처리 되는지 확인합니다.
        ...
        상대편을 파악 할 수 있어야 합니다
        현재 내 상태를 기반으로 상대를 파악할 수 있는 코드를 하나 작성합시다
        그리고 잘 가져왔는지 sout으로 출력해서 확인해봅시다.
