...
export const fetchBoardList = async (): Promise<Board[]> => {
  const response = await axiosInst.springAxiosInst.get<Board[]>(
    "/jpa-board/list"
  );
...
export const useBoardListQuery = (): UseQueryResult<Board[], unknown> => {
  ...
  const queryResult: UseQueryResult<Board[], unknown> = useQuery(
    "boardList",
    fetchBoardList,
    {
      onSuccess: (data) => {
...
export const registerBoard = async (data: {
  ...
  const response = await axiosInst.springAxiosInst.post<Board>(
    "/jpa-board/register",
    data
  );
  return response.data;
...
export const useBoardQuery = (
  boardId: string
): UseQueryResult<Board | null, unknown> => {
