...
public class RpgGame {
    final int CHARACTER_NUMBER = 3; //캐릭터 몇명
    final int MONSTER_NUM = 2;      //몬스터 몇명
    List<GameCharacter> gameCharacterList;  //캐릭터 리스트
    List<Monster> monsterList;              //몬스터 리스트

    public RpgGame() {
        gameCharacterList = new ArrayList<>();  //게임캐릭터 어레이리스트 생성

        for (int i = 0; i < CHARACTER_NUMBER; i++) {    //캐릭터 수만큼 반복
            GameCharacter gameCharacter = new GameCharacter();  //게임 캐릭터 객체 생성
            gameCharacterList.add(gameCharacter);       //생성한 객체를 게임캐릭터 어레이리스트에 추가
        }

        monsterList = new ArrayList<>();    //몬스터 "
        ...
        for (int i = 0; ; i++) {
            ...
            for (int characterIndex = 0; characterIndex < gameCharacterList.size(); characterIndex++) {
                ...
                do {
                    ...
                    targetMonsterIndex = CustomRandom.generateNumber(MONSTER_NUM - 1);  //배열은 0부터 시작해서 -1해줌
