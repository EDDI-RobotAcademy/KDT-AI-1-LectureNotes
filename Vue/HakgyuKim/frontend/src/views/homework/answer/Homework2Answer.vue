<template>
    <div>
        ...
        <p>총점: {{ playingNumber }}</p>
        <p>승자 판정: {{ result }}</p>
        <v-divider>컴퓨터와 주사위 3개를 굴립니다.</v-divider>
        <div style="text-align: left; padding: 20px;">
        <h3>컴퓨터와 주사위 3개를 굴립니다.</h3>
        <h3>첫 번째 주사위가 짝수라면 두 번째와 세 번째를 굴릴 수 있습니다.</h3>
        <h3>세 번째 주사위의 숫자가 1인 경우 상대편 점수를 3점 뺐습니다.</h3>    
...
export default {
    ...
    methods: {
        startTwoTimesDiceGame() {
            axios.get('http://localhost:7777/dice-problem/firstGame')
            .then((res) => {
        ...
        startDiceGameWithComputer() {
            axios.get('http://localhost:7777/dice-problem/secondGame')
            .then((res) => {
                this.winner = res.data.winnerName
