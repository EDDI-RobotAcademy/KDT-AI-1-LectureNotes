...
class  Led {
    ...
    매서드를 작성하는 방법
        1.public을 작성하고 소괄호를() 작성하고 중괄호{}를 작성합니다.
        ...
        숫자등등이라면 Long이나 Integer(int) 입니다.
        무엇을 반환(리턴) 하느냐에 따라 적절한 형태를 적어주면 됩니다.

    *도대체 리턴 된다는 것은 무엇을 의미 하는가?
                3 -> [   ] -> 9     (리턴타입 int)
        버튼을 누름 -> [   ] -> true  (리턴 타입 boolean)
               1  -> [   ] -> "예금" (리턴 타입 String)
          회원 정보 -> [   ]          (리턴 타입 void) (리턴하지 않음) (생성자의 경우 그냥 리턴타입이 없음! void랑은 다르다.)
                   -> [   ] -> 20    (리턴 타입 int)
     */
    ...
    /*
    사실은 지가 지를 킬 수 없기 때문에 다른 객체의 도움을 받아야한다.
