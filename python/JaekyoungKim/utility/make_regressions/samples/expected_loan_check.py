...
@bank.post("/bank-receiver")
async def can_lent_money_amount(post_income: int, post_outstanding_amount: int):
    # calculate_score 함수 호출하여 점수 계산
    total_score = calculate_score(post_income, post_outstanding_amount)

    # calculate_loan 함수에 점수를 전달하여 대출 승인 또는 거절 확인
    loan_amount = calculate_loan(total_score)

    if loan_amount:
        return {"loan_amount": loan_amount, "total_score": total_score}
    else:
        return {"message": "대출 승인 거절!"}
