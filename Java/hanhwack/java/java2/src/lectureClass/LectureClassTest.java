...
/*
       ...
       ㅣ    isTurnOn      ㅣ
        -------------------
       ㅣ    Led 생성자     ㅣ  <<<- new를 하면 이와 같은 형태가 만들어지는 것입니다.
       ㅣ    getTurnOn     ㅣ
       ㅣ    setTurnOn     ㅣ
       ---------------------
...
class Led {
    ...
    // 3. 매서드의 이름을 그옆에 작성해줍니다
    //      이름은 역시나 이 작업이 무엇을 하는지 명시적으로 알려줄 수 있는 형태가 좋습니다
    // 4. 소괄호 뒤에는 입력타입을 작성 없다면 비워놓도록
    // 5. 중괄호 내에서는 실제 매서드 이름에 해당하는 작업을 진행하면 됩니다.
    //      이 때 단순히 작업만 하고 정보 반환이 없다면 리턴 타입은 void입니다
    //      참/가짓을 반환한다면 Boolean(boolean)입니다
    ...
    // 1 -> [   ] -> "예금" (입력 타입 int)
    //  -> [     ] -> 20 (입력 타입 void) == 정확히는 입력하지 않음을 의미함
    // 참/거짓 -> [     ] ->  (입력 타입 boolean)
...
public class LectureClassTest {
    public static void main(String[] args) {
        ...
        // 필요하면 조립하여 관리하자는 뜻을 가지고 있습니다

        // 하지만 모든 정보를 하나의 클래스에서 관리하게 되는 경우
        // 객체가 비대해지면서 해당 객체가 어떤 목적을 가지고 있었는지 목적성을 잃게 됩니다
        // Domain이란 관점은 이렇게 클래스가 어떤주제에 집중을 하고 있는지를 본다고 생각하면 됩니다.
        ...
        // 전구(LED)를 가는 상황을 생각해봅시다
        // 생성자 호출하는  부분 [ new Led() ]
        System.out.println("생성자 호출 전");
        final Led led = new Led();             // <- 초기 생성(꺼짐 - isTurnOn : false)
        // 데이터 타입 적는 위치에 class이름이 오는 이유?
        // 클래스라는 것 자체가 커스텀 데이터 타입이기 때문입니다
        // 직접 커스텀 할 수 있는 데이터 타입이 클래스라고 보면 됩니다
