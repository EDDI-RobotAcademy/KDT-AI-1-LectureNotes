...
//Led 클래스는 불이 켜졌다 혹은 꺼졋다만 관리하면 됩니다.
// 클래스의 형태는 다음과 같습니다.
/*
    ----- Led 객체 -----
    |     isTurnOn     |
...
class Led{
    ...
    // 그리고 아래와 같은 리턴 타입이 없습니다.
    // 만드는 규칙이라면 아래와 같습니다.
    // 1. public 을 적고 클래스 이름을 적은 이후 소괄호()를 작성 후 중괄호{}를 생성합니다.
    // 2. 만약에 외부에서 값을 입력받을 것이라면 소괄호에 입력받을 형태를 작성합니다.
    ...
    // 매서드를 작성하는 방법
    // 1. public을 작성하고 소괄호()를 열고 중괄호{}를 작성합니다.
    //  2. 리턴 타입을 public 옆에 작성합니다.
    // 3. 매서드의 이름을 그 옆에 작성해줍니다.
    //  이름은 역시나 이작업이 무엇을 하는지 명시적으로 알려줄 수 있는 형태가 좋습니다.
    // 4. 중광호 내에서는 실제 매서드 이름에 해당하는 작업을 진행하면 됩니다.
    // 이 떄 단순히 작업만 하고 정보반환이 없다면 리턴 타입은 void 입니다.
    // 참/거짓을 반환한다면 Boolean(boolean)입니다.
    // 숫자등등이라면 Long(long)이나 Integer(int)입니다.
    // 무엇을 반환(리턴) 하느냐에 따라 적절한 형태를 적어주면 됩니다.

    // * 도대체 리턴 된다는 것은 무엇을 의미하븝가?
    //       3 -> [   ] -> 9 (리턴 타입 int)
    ...
    // 사실은 지가 지를 켤 수 없기 떄문에 다른 객체의 도움을 받아야함
    ...
    // Request 객체는 Domain Service에서 처리하기 적합한 형태인 Request로 변환
...
public class LectureClassTest {
    public static void main(String[] args) {
        ...
        // Domain이라는 관점은 이렇게 클래스가 어떤 주제에 집중을 하고 있는지를 본다고 생각하면됩니다.
        // 즉 내가 집중하는 주체가 무엇인가를 알 수 있도록 예쁘게 잘 표현해주는 것을 oop라 봐도 무방합니다.
