...
public class ArrayTest {
    public static void main(String[] args) {
        ...
        for (int i = START; i < numberArray.length; i++) {
            ...
            // 메모리 공간이라는 것이 생소하다면
            // 아래와 같이 박스들이 일렬로 나열되어 있는 상황르 생각해도 됩니다.

            //  0   1   2   3   4
            // [  ][  ][  ][  ][  ]

            // 주의할 부분이라면 배열의 시작이 0붙터라는 것에 주의를 해주세요
            ...
            // 그러므로 numberArray[0]은 숫자 1을 표ㅕㄴ하고
        ...
        // foreach의 사용법
        // 1. 배열의 데이터 타입을 작성합니다 (여기서 int)
        // 2. 배열의 원소를 표현할 이름을 적당히 지정합니다 (여기선 num)
        // 3. 콜론 하나 찍습니다 (:)
        // 4. 정보를 하나씩 꺼내올 배열을 쩍어줍니다 (여기선 numberArray)
        for (int num: numberArray) {
            System.out.println("배열 원소 출력: " + num);
        }

        // 비즈니스가 확장됨에 따라 서비스가 커졌고
        // 그에 따라 데이터 규모도 커졌다 (여기선 배열에 정보가 많다고 가정)
        // 이런 상황에서 for(초기화; 조건; 증감)과 foreach가 있다면 누가 더 좋을까??

        // 의견 1 -> 초기화 조건들이 있어 향후 변경에 유리하므로 전자선호
        // 의견 2 -> 전자는 조건식을 계속 검사해야하므로 후자선호

        // 이러한 이유 때문에 사실 테이터를 요청하면
        // 뭉테기로 쏘지 않고 필요한 정보만을 선별해서 쏘게 됩니다.
        // 가령 AWS에서 1000만명 분량의 데이터를
        // 검색 한 번에 다 가져오게 되면 요금 폭탄맞고 회사 망합니다
        // 그래서 서비스 고나점에선 이런 조각 내기 개념으로서 페이징이라는 것을 합니다
        // OS레벨에서 이야하기는 Paging과는 다른 개념이니 주의합시다.
        // (여기서 얘기하는 paging은 데이터 조각내기 관점이라 보면 됩니다)
    }
}
