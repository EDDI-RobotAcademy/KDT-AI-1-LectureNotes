<template>
    <div>
        <!-- 
            ...
            button typr="submit"이 동작하지 않게 만들었음
            실제 버튼 submit 작업이 onSubmit() 매서드에서 동작하게 만들었음
        -->
        <form @submit.prevent="onSubmit">
        <table>
            <tr>
                <!-- 터미널 bash에 npm install axios --save-dev 입력 -->
                <th>이름</th>
...
export default {
    ...
    methods: {
        /*
        위쪽의 template 파트 html은 전부 v-medel로 연결되어 있어 사용자 입력에 따라 값이 바뀝니다.
        그러므로 this를 통해서 입력된 값들을 전달 할 수 있습니다.
        this가 data()에 있는 name, major, age를 모두 포함하기 때문입니다.
        const { 정보 } = this 를 통해서 특정 정보를 객체화 할 수 있습니다.
        ...
        */
        onSubmit () {
            ...
            // 보호해야 하는 정보는 GetMapping으로 설정해주면 안된다.(사용자정보)
