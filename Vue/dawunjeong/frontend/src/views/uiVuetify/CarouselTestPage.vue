<template lang="">
<v-container>
    <div>
        <v-carousel :cycle="cycle" :interval="interval" :height="height" v-model="carouselModel">
            <v-carousel-item v-for="(picture, idx) in bannerPictures" :key="idx">
                <v-sheet :height="height" tile>
                    <v-row class="fill-height" align="center" justify="center">
                        <v-img :src="picture.img" :height="height" contain/>
                    </v-row>
                </v-sheet>
            </v-carousel-item>
        </v-carousel>
    </div>
</v-container>
</template>

<script>
export default {
    data () {
        return {
            carouselModel: false,
            bannerPictures: [
                { img: require("@/assets/downloadImg/naruto.png")},
                { img: require("@/assets/downloadImg/mario.png")},
                { img: require("@/assets/downloadImg/dragonquest.png")},
                { img: require("@/assets/downloadImg/ff7.png")},
            ],
            cycle: true,
            interval: 2500,
            height: 600,
        }
    }
}
</script>

<style lang="">
    
</style>