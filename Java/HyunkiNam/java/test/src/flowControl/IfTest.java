...
public class IfTest {
    public static void main(String[] args) {
        ...
        // if 문을 만드는 방법
        // 1. if를 작성하고 소괄호()를 작성하고 중괄호를{}를 작성합니다.
        ...
        // 조건식을 첫번째 if가 만족되지 않았을 때 else if를 보게 되므로
        // 기본적으로 해당 else if 에서는 if의 조건 또한 만족하지 않음을 내포하게 됩니다.
        // 그리고 그 다음 else if 에서는 맨 처음 if가 만족하지 않고, 그 다음 else if를 만족하지않고,
        // 그리고 현재의 else if 조건을 만족해야합니다.

        // 그러므로 depth(깊이)가 깊어질수록 코드를 파악하기 위한 혼동이 가중된다는 문제가 있습니다.
        // 이와 같은 이유때문에 코드를 작성할 때 if, else if, else if 보다는
