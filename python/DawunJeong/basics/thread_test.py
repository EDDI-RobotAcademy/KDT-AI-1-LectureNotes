...
# OS 마다 Process 및 Memory 관리 매커니즘이 상이하여 일부 차이점이 존재함
# 대표적으로 Windows vs Linux, Unix(macOS) 형태를 띰
# Windows는 프로세스 및 스레드 모델이 상당히 좋지 못함
# 그래서 대부분 서버를 운영하는 경우 Windows 서버를 사용한다 하면 '?' 때리는 경우가 많음

# 반면 Linux와 그의 형인 Unix는 유사한 매커니즘을 사용하고 있음
...
# S사의 모든 직원들도 자기 자신의 Process가 있지만 소속은 S사였음
...
# (근데 바이두라는 중국 구글 같은 사이트에 가서 한국인 주민등록번호를 검색하면
# 이름이랑 주민등록번호 리스트가 스크롤 끝도없이 검색되서 나오는 것은 덤 ㅋㅋㅋㅋ)

# 부가적으로 ps -ef에서 TTY에 '?'라고 나오는 것은 데몬 프로세스라고 부름
# 실질적으로 서비스 구동되는 애들은 전부 '?' 라고 표기됨
# 대표적으로 우리가 사용하는 nginx 서버도 '?'로 표기되는 것을 볼 수 있음
# 또한 java 명령을 통해서 jar 파일을 실행하여 구동되는 서비스 또한 '?'로 표기되는 것을 볼 수 있음
# 마찬가지로 docker 또한 서비스로 구동되고 있으므로 '?'라고 표기됨

# ps -eLf | grep docker 라고 입력하면 docker 관련 thread 정보도 확인이 가능함
# 입력한 결과는 아래와 같은 것들을 볼 수 있음
# 첫 번째 라인에서 같은 PID를 가지고 있는 13개의 결과를 확인 할 수 있음
...
# 10^6 = 1,000,000(백만개)의 명령을 처리할 수 있습니다.
