...
public class LambdaTest {
    ...
    /*
        ...
        DDD를 잘하려면 전제 조건이 TDD가 깔려야 한다.
        DDD의 근본적인 목적은 결국 관심사를 분리해서 복잡성을 낮추는데 있다.

        왜(Why) ?
        왜에 대해 생각하기 이 전에 TDD는 Test를 먼저 작성하고
        이 'Test에 대응하는 코드를 만들어보자 !' 가 핵심이라 보면 된다.

        이 내용을 기반으로 'Why' 에 대한 유추를 해보자면 ?
        - Test를 작성하면서 Domain을 파악할 수 있기 때문이다. (즉, Domain 찾는 것을 서포트 한다.)

        무엇이든 처음 접근하는 것은 매우 어렵습니다.
        하지만 개발자는 처음 접근하는 것에 도전 ? 하여 깨부수는 작업입니다.
        (물론 개발자도 레벨이 다양하기 때문에 요쪽은 소위 천상계라고 부릅니다.)
        그리고 이 천상계로 가기 위해 반드시 필요한 작업들이 TDD, DDD 입니다.

        실제 Java 코드를 작성할 때도 문제 전체를 한 번에 만들지 않고
        여기서 문제 자체가 요구하는 사항들을 정리하였습니다.
        - 여기서 또 팁이라면 요구 사항을 정리할 때 완벽할 필요가 없습니다.

        오히려 위의 요구 사항을 완벽하게 만드려다가 아무것도 못하고 나락으로 향하게 됩니다.
        이것의 대표적인 특징 중 하나가 회의만 많이 하는 회사나 그룹입니다.
    ...
    */
    ...
    public void 두개의_숫자_더해서_평균_계산 () {
        ...
        collection.add(() -> 5); // () 를 익명함수 (람다)라고 부른다.
        collection.add(() -> 9);
