...
public class VariableTest {
    public static void main(String[] args) {
        ...
        //결론적으로 변수를 만들 때 중요한 것은
        // 변수 이름인데 명시성을 통해 함께작업하는 팀원들에게
        //이것이 무엇을 의미하는 것인지 명확하게 전달하기 위한 목적이 가장중요합니다
        int appleCount = 3;
        int grapeCount = 5;
        int totalCount = appleCount + grapeCount;

        // 문자열 + 숫자의 경우엔 앞의 문자열이 나왔기 때문에
        // 자동으로 숫자를 무자열 처리하여 화면에 출력합니다
        System.out.println("totalCount = " + totalCount);

        //final을 사용하는 이유: 우선 상수로 고정시킬 수 있다는 이점이 있음
        //상수로 고정시킬 수 있다는 것의 이점은 무엇인가?

        //1. 아래있는 [비교대상]을 보면 3.3f를 직접 기입하고 있으므로
        //  향후 프로그램이 커지면 직접 3.3f를 작성한 부분을 모두찾아서
        //  TAX변경에 따라 모든 코드를 수정해야하는 불편함이 발생합니다.
     ...
     */
        ...
        //TAX = TAX +4;
        //위코드는 TAX가 final이기 때문에
        //새로운 값을 대입하거나 덧셈 뺄셈 등등이 불가능합니다
        //결론적으로 입력되는 값을 변경하지 못하게 막을으로서
