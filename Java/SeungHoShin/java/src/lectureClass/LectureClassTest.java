...
// Led 클래스를 불이 켜졌다 혹은 꺼졌다만 관리하면 됩니다.
...
class Led{
    ...
    // 3. 매서드의 이름을 그옆에 작성합니다.
    // 이름은 역시나 이작업이 무엇을 하는지 명시적으로 알려줄 수 있는 형태가 좋습니다.
    //4. 중괄호 내에서는 실제 매서드 이름에 해당하는 작업을 진행하면 됩니다.
    // 이때 단순히 작업만 하고 정보반환이 없다면 리턴 타입은 void입니다.
    // 참/거짓을 반환한다면 Boolean(boolean)입니다.
    // 숫자등등이라면 Long(long)dlsk Integer(int)입니다.
    // 무엇을 반환(리턴) 히느냐에 따라 적절한 형태를 적어주면 됩니다.
    //* 도대체 리턴 된다는 것은 무엇을 의미 하는가?
    //         3->[   ] ->9(리턴 타입 int)
    // 버튼 누름 -> [   ] -> true(리턴 타입 boolean)
    //     1   -> [    ] ->"예금"(리턴 타입 string)
    //   회원정보 ->[    ]        (리턴 타입 void) == 정확히는 리턴하지 않음을의미함
    //             [   ]  ->20  (리턴 타입 int)
    ...
    //Domain Service 를 만들어서 얻는 이점은
    // 비지니스 관점을 좀 더 명확하게 만들어준다는 이점이 있습니다.
    // 실제로 전구를 키고 끄는 작업을 가지고 비지니스를 할만한 것들이 많지는 않지만
    ...
    //RequestForm 객체는 Domain Service에서 처리하기 적합한 형태인 Requset로 변환
    ...
    // *입력 관점에서 살펴봅시다.
    //         3->[   ] ->9(입력 타입 int)
    // 버튼 누름 -> [   ] -> true(입력 타입 boolean)
    //     1   -> [   ] ->"예금"(입력 타입 int)
    //            [   ]  ->20   (입력 타입 void)
    //    참/거짓      [   ]  ->20  (입력 타입 boolean)
...
public class LectureClassTest {
    public static void main(String[] args) {
        ...
        // Domain 이라는 관점은 이렇게 클래스가 어떤 주제에 집중을 하고 있었는지 본다는 생각하면 됩니다.
        // 즉 내가 집중하는 주제가 무엇인가를 알 수 있도록 예쁘게 잘 표현해주는 것을 OOP라 봐도 무방합니다.
        // 전구(LED)를 키는 상황을 생각해봅니다.
