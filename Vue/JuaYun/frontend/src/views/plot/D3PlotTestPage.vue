...
export default {
    ...
    computed: {
        ...
        line () {
            ...
            // 그리고 실제로 x는 xScale을 통해서 실행이 됨
            console.log('path()에서 불려짐: 넌 언제 그려지니?') // 콘솔설정 후 mounted를 주석처리
            return d3.line()
                    // d는 실질적인 data를 의미함
                    // i는 해당 data의 index를 의미함
                    // 콘솔을 사용해 데이터가 나오는지 확인해보기
                    // .x((d, i) => { this.xScale(i) }, console.log("i: " + i))
                    // .y((d) => { this.yScale(d) }, console.log("d: " + d))
                    .x((d, i) => this.xScale(i))
                    .y((d) => this.yScale(d) )
        },
        xScale () {
    ...
    mounted () {
        ...
        // 주식차트같이 생각하면됨
        // 그래프의 필요한 값을 설정하는 x축을 만들어줌
        g.append("g")    
            // transform은 평행 이동, 회전 변한 등등을 수행할 수 있음
            // translate(0, heigth)와 같으므로
        ...
        g.append("path")
            .datum(this.reg_data)
            // 위에서도 40만큼 공간을 확보하므로 전체 구성이 깨지지 않으려면 
            // 여기서도 40만큼 수평이동을 시켜서 그래프 전체의 윤곽이 흐트러지지 않게 구성함
            .attr('transform', `translate(40, 0)`) // price값이 나오지 않아 추가된 코드
            .attr("fill", "none")
