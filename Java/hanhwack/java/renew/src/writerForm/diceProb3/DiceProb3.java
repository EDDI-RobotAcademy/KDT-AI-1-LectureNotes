...
public class DiceProb3 {
    ...
    public static void main(String[] args) {
        // 조건 : 향상 먼저해야 하는 작업은 이런식으로 문제의 조건들을 분리하는 것입니다.
        ...
        // 조건은 정말 저 조건들을 싲ㄹ제 구현해야 하는 상황이 되었을 때 처리합시다
        ...
        // 우선 게임 객체 생성자에서 뭐 필요한가?
        // 2번의 게임플레이()를 만들어보자
        // 일단 플레이어가 주사위를 굴려야겠네
        // 이 상태에ㅔ서 진행하려고 했더니 플레이어와 주사위가 없네
        // [1] 초기에 셋팅할까 ? <- 생성자의 1번에 필요한 것들을 배치
        ...
        // 짝수로 판정되는 경우에만 나머지 2개를 돌리도록 코드를 변경!!

        // 다시 구동합니다
        // 짝수인 경우에만 잘 나오는지 확인합니다
        // 그리고 승자 판정도 잘 동작하는지 확인합니다

        // 조건을 다시 보니까 특수 기능이 동작하지 않는것을 확인
        ...
        // 어떤 경우엔 잘 되고 어떨 때는 에러가 발생
        // 아마도 에러 메시지를 읽어보면 NullPointExecption
        // 혹은 index OutofBound 같은 것에 해당할 것입니다

        // 안나오는 이유는?
        // 살펴보니 도 번째와 세 번째 주사위가 안나올 수 있었네?
        // 이 부분을 고려하지 않았군
        // 에외처리 추가해야겠네
        // 세 번째 주사위 숫자를 확인하기 전에
        // 해당 주사위가 존재하는지 먼저 확인해봐야겠구나

        // 실제 읽은 숫자가 어던 조건에 대응하는지 살펴봐야합니다
        // 우선 숫자 4번부터 보기로 했으니까 사망 판정부터 때립시다

        // 주사위 숫자를 더하다보면 음수가 나올 수 없으니까 그냥 음수값 큰거 주자
        // 숫자가 4가 나오면 사용자 점수에 대략 -444점 같은 형태로 점수를 줍니다
        // 여기까지 처리하고 실제 숫자4가 나올때 패배 처리가 되는지 확인합니다

        // [ 숫자가 3이라면 자가 버프 2점 ]
        // 이제 다음으로 만만한 자가 버프를 해봅시다
        // 세 번째 주사위 숫자 3을 확인하면 자신의 점수에 2점을 추가해줍니다
        // 이후 다시돌려서 3이 나왔을 때 2점이 추가되는지 확인합니다
        // 그와 동시에 숫자 4에 대한 패배 처리도 확인합니다.
